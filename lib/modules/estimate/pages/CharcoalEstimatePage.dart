...
import 'EstimateDetailPage.dart';
...
class CharcoalEstimateListPage extends StatefulWidget {
  const CharcoalEstimateListPage({super.key});

  @override
  _CharcoalEstimateListPageState createState() =>
      _CharcoalEstimateListPageState();
}

class _CharcoalEstimateListPageState extends State<CharcoalEstimateListPage> {
  ...
  Future<void> _loadEstimates() async {
    final data = await CDatabaseHelper.fetchAllCharcoalEstimates();
    setState(() {
      _estimates = data;
    });
  }
  ...
  void _viewDetails(int estimateId) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (context) => EstimateDetailPage(
            estimateId: estimateId), // Updated page navigation
      ),
    );
  }

  Future<bool> _deleteEstimate(int id) async {
    final response = await CDatabaseHelper.deleteEstimate(id);
    if (response) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text("Saved Charcoal Estimates"),
